...
while backtomenu == 'y':
    ...
    if choice == 1:
        ...
        if modify == 1:
            uid = input('Enter student ID to edit')
            std_fname = input('Change firstname to : ')
            cursor.execute("update PHCDC_StudentPersonal set student_fname = ? where student_id = ?;",std_fname,uid)
            print('Record has been updated successfully')
        elif modify == 2:
            uid = input('Enter student ID to edit')
            std_lname = input('Change lastname to : ')
            cursor.execute("update PHCDC_StudentPersonal set student_lname = ? where student_id = ?;",std_lname,uid)
            print('Record has been updated successfully')
        elif modify == 3:
            uid = input('Enter student ID to edit')
            ...
            cursor.execute("update PHCDC_StudentPersonal set student_add = ? where student_id = ?;",std_add,uid)
            print('Record has been updated successfully')
        elif modify == 5:
            uid = input('Enter student ID to edit')
            std_city = input('Change city to : ')
            cursor.execute("update PHCDC_StudentPersonal set student_city = ? where student_id = ?;",std_city,uid)
            print('Record has been updated successfully')
        elif modify == 6:
            uid = input('Enter student ID to edit')
            std_postal = input('Change postal to : ')
            cursor.execute("update PHCDC_StudentPersonal set student_postal = ? where student_id = ?;",std_postal,uid)
            print('Record has been updated successfully')
        elif modify == 7:
            uid = input('Enter student ID to edit')
            std_state = input('Change state tu : ')
            cursor.execute("update PHCDC_StudentPersonal set student_state = ? where student_id = ?;",std_state,uid)
            print('Record has been updated successfully')
        elif modify == 8:
            uid = input('Enter student ID to edit')
            std_email = input('Change email to : ')
            cursor.execute("update PHCDC_StudentPersonal set student_email = ? where student_id = ?;",std_email,uid)
            print('Record has been updated successfully')
        elif modify == 9:
            uid = input('Enter student ID to edit')
            std_phono = input('Change phone number to : ')
            cursor.execute("update PHCDC_StudentPersonal set student_phono = ? where student_id = ?;",std_phono,uid)
            print('Record has been updated successfully')
        else:
        ...
        total_m = int(input('Total mark : '))
        result = input('result : ')
        grade = input('grade : ')

        cursor.execute("Insert PHCDC_StudentMark(std_id,assignment_mark,miniproject_mark,test_mark,total_mark,result,grade) VALUES (?,?,?,?,?,?,?)",
            (std_id, assignment_m, miniproject_m, test_m, total_m, result, grade))
        print('Record has been inserted successfully')
        ...
        uid = input('Enter user ID')
        cursor.execute("SELECT * FROM PHCDC_StudentPersonal LEFT JOIN PHCDC_StudentMark ON PHCDC_StudentPersonal.student_id=PHCDC_StudentMark.std_id where student_id = ?", uid)
        result = cursor.fetchall()
        # print(type((result)))
        for x in result:
            print(x)
