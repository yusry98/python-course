...
"""
This Project on Hotel Management is a general software developed (using Python3.9 and SQL server2019) to simplify hotel operations by automating them.
In this project, “Malaysian Best Hotel” is the project’s hotel name. It covers major aspects of hotel management;
it could perform the following operations- Hotel Booking, Provide you with Hotel Rooms Info, Room Service, Billing and Record-Keeping.
...
# Home Function
def Home():
    print("\n\t\t\t\t\t\t WELCOME TO Malaysian Best Hotel\n")
    print("\t\t\t 1 Booking\n")
    print("\t\t\t 2 Rooms Info\n")
...
def Booking():
    ...
    # type and it's price
    if ch == 0:
        print(" 1. Standard Non-AC - RM. 150")
        print(" 2. Standard AC - RM. 200")
        print(" 3. 3-Bed Non-AC - RM. 350")
        print(" 4. 3-Bed AC - Rs. 400")
        ch = int(input("->"))
    if ch == 1:
    ...
    for n in range(0, i):
        if ph == phno[n]:
            ...
            if p[n] == 0:
                ...
                print("\n            Pay For Hotel Malaysian Best Hotel")
                print("  (y/n)")
                ch = str(input("->"))
...
cursor = conn.cursor()  # initialization of the cu
cursor.execute("SELECT * FROM restaurant where c_ic = '980408-08-6407'")
result = cursor.fetchall()
total = 0
for y in result:
    total = np.sum([y[1] for y in result])
    print(total)
